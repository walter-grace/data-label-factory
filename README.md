...
- [`create-agent-gateway/`](./create-agent-gateway) — npm template to scaffold a new gateway for your own paid API (not yet published; tarball in folder)
...
Full setup + architectural decisions documented in [`agent-gateway/README.md`](./agent-gateway/README.md). The same skeleton, stripped of DLF-specific routes, is packaged as [`create-agent-gateway`](./create-agent-gateway) — a reusable template for any pay-per-call agent API.
