...
// Pricing set to cover real infra costs with margin.
//
...
//
// Predict + train are cold-start-sensitive. Until we keep workers warm
// (workersMin>=1 ≈ $25/mo floor), we price for the cold case so one-and-
// done customers don't extract more in infra than they paid.
const PRICE_MCENTS = {
  crawl_per_page: 50,      // $0.0005 / page   — 4× margin
  gather: 100,             // $0.001  / call   — ~100% margin
  label_per_image: 200,    // $0.002  / image  — 2-100× margin
  train_yolo: 5000,        // $0.05   / job    — covers cold RunPod + margin
  predict_per_image: 300,  // $0.003  / img    — covers cold RunPod + margin
} as const;
...
<body>
  ...
  <ul>
    ...
    <li><code>POST /v1/signup</code> — get a key via x402 (0.10 USDC on Base → 20,000 mcents)</li>
  </ul>
  <h2>For humans</h2>
...
async function handleLlmsTxt(): Promise<Response> {
  return textResponse(
    [
      ...
      "",
      "### POST /v1/train-yolo/start — Launch YOLOv8n training on RunPod (2000 mcents/job)",
      ...
      "",
      "### POST /v1/predict/:job_id — Run trained model on a new image (20 mcents)",
      ...
      "- x402 signup: 0.10 USDC on Base → 20,000 mcents ($0.20, 2× payment) starter balance.",
...
async function handlePredict(req: Request, env: Env, jobId: string): Promise<Response> {
  ...
  const refundAndFail = async (status: number, errMsg: string, extra: any = {}) => {
    const now = Date.now();
    if (canIssueRefund(auth.record, now)) {
      const window = (auth.record.refunds_window || []).filter((t) => now - t < REFUND_WINDOW_MS);
      window.push(now);
      auth.record.refunds_window = window;
      rollbackChargeAndXp(auth.record, PRICE_MCENTS.predict_per_image, "predict");
      await writeKey(env, auth.key, auth.record);
      return json({
        ok: false, error: errMsg, charged: false, refunded: true,
        balance_mcents: auth.record.balance_mcents,
        model_job_id: jobId, ...extra,
      }, { status });
    }
    // Refund cap hit — still charge but tell the caller.
    return json({
      ok: false, error: errMsg, charged: true, refunded: false,
      balance_mcents: auth.record.balance_mcents,
      model_job_id: jobId, ...extra,
    }, { status });
  };
  ...
  if (ownerKey && !isOwner && !isPublic) {
    // No refund yet (we charged up front in authAndCharge). Roll back cleanly.
    rollbackChargeAndXp(auth.record, PRICE_MCENTS.predict_per_image, "predict");
    await writeKey(env, auth.key, auth.record);
    return json({
      ok: false,
  ...
  while (Date.now() < deadline) {
    ...
    if (st === "COMPLETED") {
      ...
      if (pub?.published && ownerKey && !isOwner) {
        ...
        try {
          const ownerRec = await readKey(env, ownerKey);
          if (ownerRec) {
            ownerRec.balance_mcents += ownerShare;
            await writeKey(env, ownerKey, ownerRec);
          }
          const m = await readPublishedModel(env, jobId);
          if (m) {
            m.uses += 1;
...
async function handleTrainStart(req: Request, env: Env): Promise<Response> {
  ...
  if (!resp.ok || !data?.job_id) {
    ...
    if (canIssueRefund(auth.record, now)) {
      ...
      window.push(now);
      auth.record.refunds_window = window;
      rollbackChargeAndXp(auth.record, PRICE_MCENTS.train_yolo, "train");
      await writeKey(env, auth.key, auth.record);
      return json({
        ok: false,
...
// Starter balance set below to 20,000 mcents ($0.20, 2× payment). XP head start = 0.

const SIGNUP_PRICE_ATOMIC = "100000"; // 0.10 USDC (6 decimals)
// Starter credit set to 2× the payment. Bigger bonuses invited one-and-done
// customers to extract more in RunPod time than they paid. 2× keeps onboarding
// attractive (the first train + predict fits) while protecting against the
// worst-case cold-start burn.
const SIGNUP_STARTER_MCENTS = 20000; // $0.20
    ...
    accepts: [
      {
        ...
        resource,
        description: "DLF agent API key — $0.10 unlocks 20,000 mcents ($0.20 starter balance)",
